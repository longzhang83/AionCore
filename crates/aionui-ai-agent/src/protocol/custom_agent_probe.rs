    ...
    let outcome = match protocol.new_session(NewSessionRequest::new(std::env::temp_dir())).await {
        Ok((response, legacy_models)) => {
            ...
            let models = legacy_models
                .as_ref()
                .and_then(crate::manager::runtime::legacy_runtime_model::LegacySessionModelState::from_state_value);
            ProbeOutcome::Ok(
                crate::manager::runtime::runtime_catalog_forwarder::catalog_partial_from_session_new(
